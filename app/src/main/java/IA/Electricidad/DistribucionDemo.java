...
public class DistribucionDemo {
    ...
    private static void inicializar() {
        ...
        for (int j = 0; j < maxP; j++) {
            od.addPaquete(j, "p-" + String.valueOf(j), r.nextInt(2) + 1, r.nextInt(9) + 1, r.nextInt(4000) + 1000,
                    r.nextInt(45) + 5);
        }
    }
    ...
    private static void DistHillClimbingSearch(Distribucion dist) {
        ...
        try {
            Problem problem = new Problem(dist, new DistribucionSuccessorFunction1(), new DistribucionGoalTest(),
                    new DistribucionHeuristicFunction4());
    ...
    private static void printDates() {
        ...
        for (int i = 0; i < od.getNumOferta(); i++) {
            System.out.println("Id:" + i + "\tCompany:" + od.getPCompany(i) + "\tCo2:" + od.getPCo2(i) + "\tkWh:"
        ...
        for (int j = 0; j < od.getNumDemanda(); j++) {
            System.out.println("Id:" + j + "\tNom:" + od.getCNom(j) + "\tCo2Min:" + od.getCCo2Min(j) + "\tkWh:"
                    + od.getCKWh(j) + "\tPrecio:" + od.getCPrecioMax(j) + "\n");
        }
    }
