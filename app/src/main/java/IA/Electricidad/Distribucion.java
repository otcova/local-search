...
public class Distribucion {
    ...
    public boolean puedeReasignar(int idConsumidor, int idPaquete) {
        // El paquete ya est� asignado a este consumidor
        if (cAsignaciones[idConsumidor] == idPaquete)
            return false;

        // No queda suficiente energia
        if (pKWHVendido[idPaquete] + od.getCKWh(idConsumidor) > od.getPKWhTotal(idPaquete))
            return false;

        // El consumidor no puede pagar el paquete
        if (od.getCPrecioMax(idConsumidor) < od.getPPrecio(idPaquete))
            return false;

        // El paquete es demasiado contaminante
        if (od.getCCo2Min(idConsumidor) > od.getPCo2(idPaquete))
            return false;
    ...
    public boolean equals(Object o) {
        // Casteamos la entrada a Distribuci�n
        Distribucion dist = (Distribucion) o;

        // La direcci�n de memoria del objeto de Datos es la misma
        if (dist.od != this.od)
            return false;

        // La cantidad de consumidores y paquetes es la misma
        if (this.cAsignaciones.length != dist.cAsignaciones.length)
            return false;
        if (this.pKWHVendido.length != dist.pKWHVendido.length)
            return false;

        // Para cada consumidor el paquete asignado es el mismo
        for (int i = 0; i < this.cAsignaciones.length; i++) {
            if (this.cAsignaciones[i] != dist.cAsignaciones[i])
                return false;
        }

        // Para cada paquete la cantidad de kWh vendida es la misma
        for (int i = 0; i < this.pKWHVendido.length; i++) {
            if (this.pKWHVendido[i] != dist.pKWHVendido[i])
                return false;
        }
