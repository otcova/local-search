...
public class Representacio {
	...
	public Representacio(int numCentrals, int numRepetidors, int amplada, int altura, int alfa, int beta, int gamma,
			int maxRepetidors) {
	...
	public void solucioInicial3() {
		...
		ops[4] = true;
		ops[5] = true;
		problem = new Problem(this, new CentralsGeneradorEstats(1000, ops), new CentralsEstatFinal(),
				new CentralsFuncioHeuristica4());
		...
		for (int i = 0; i < this.getNumCentrals(); i++) {
			if (((Vector) this.estat.elementAt(i)).size() < 2) {
				...
				for (int j = 0; j < dest.length && !fi; j++) {
					...
					} else if (dest[j].id < this.getNumCentrals() && ((Vector) this.estat.get(dest[j].id)).size() < 4
							&& ((Vector) this.estat.get(dest[j].id)).size() != 1) {
						((Vector) this.estat.elementAt(i)).addElement(new Integer(dest[j].id));
						((Vector) this.estat.elementAt(dest[j].id)).addElement(new Integer(i));
						fi = true;
					}
	...
	public int[][] getPlano() {
		...
		for (int i = 0; i < numCentrals + numRepetidors; i++) {
			if (vNodos[i].getEsCentral())
				...
				tablero[vNodos[i].getCordX()][vNodos[i].getCordY()] = 2;
		}
		return tablero;
	}
	...
	public void printVNodos() {
		...
		for (int i = 0; i < vNodos.length; i++)
			System.out.println(vNodos[i].getCordX() + " " + vNodos[i].getCordY() + " " + vNodos[i].getEsCentral());
	...
	public boolean interConexio(int origen, int antic, int nou) {
		...
		if (((Integer) ((Vector) this.estat.get(nou)).get(0)).intValue() >= numCentrals) {
			if (((Vector) this.estat.get(nou)).size() - 1 >= this.NUM_CONEXIONS_REPETIDOR)
				return false;

		} else {
			nouescentral = true;
			if (((Vector) this.estat.get(nou)).size() - 1 >= this.NUM_CONEXIONS_CENTRAL)
				return false;
		}

		if (((Integer) ((Vector) this.estat.get(antic)).get(0)).intValue() >= numCentrals) {
			if (((Vector) this.estat.get(antic)).size() == 3)
				return false;
		}

		if (((Vector) this.estat.get(origen)).contains(new Integer(nou)))
			return false;
		if (((Vector) this.estat.get(antic)).size() < 3)
			return false; // Quedaria no connex.
	...
	private boolean potArribar(int indx, int desti, boolean[] arribats) {
		// this.printSolucio();
		// System.out.println("mi listaa es ->
		// "+((Vector)this.estat.get(indx)).get(0).toString()+" el seg ->
		// "+((Vector)this.estat.get(indx)).get(1).toString());
		for (int i = 1; i < ((Vector) this.estat.get(indx)).size(); i++) {
			...
			if (arribats[actual] == false) {
				...
				if (actual == desti)
					...
					// System.out.println("potArribar(actual,desti,arribats,indx) -> "+ actual+" "+
					// desti+" "+ arribats.toString()+" "+ indx+" ");
					if (potArribar(actual, desti, arribats))
						return true;
				}
	...
	private int getError(int i, int j, int heur) {
		...
		// PROVA -3-
		if (heur == 3)
			return distance * sumconnexions;

		return 0;
	}

	private int getDistance(int i, int j) {
		return Math.abs(vNodos[i].getCordX() - vNodos[j].getCordX())
				+ Math.abs(vNodos[i].getCordY() - vNodos[j].getCordY());
