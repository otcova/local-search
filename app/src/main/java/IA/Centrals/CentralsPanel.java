...
public class CentralsPanel extends javax.swing.JPanel {
	...
	private static Color[] vcolors = { Color.DARK_GRAY, Color.BLUE, Color.CYAN, Color.ORANGE, Color.PINK, Color.GREEN,
			Color.YELLOW, Color.MAGENTA, Color.RED, Color.WHITE };
	...
	public void paintComponent(Graphics g) {
		...
		for (int i = 0; i < amplada + 1; i++) {
			Line2D line1 = new Line2D.Double((i * sep) + gapX, gapY, (i * sep) + gapY,
					boardSize - (sep * (max - altura)) + gapY);
			g2.draw(line1);
		}
		for (int i = 0; i < altura + 1; i++) {
			Line2D line2 = new Line2D.Double(gapX, (i * sep) + gapY, boardSize - (sep * (max - amplada)) + gapX,
					(i * sep) + gapY);
			g2.draw(line2);
		}
		if (init) {
		...
		if (sol != null) {
			...
			for (int i = 0; i < this.sol.size(); i++) {
				...
				if (v.size() != 0) {
					...
					for (int j = 1; j < v.size(); j++) {
						if (Integer.parseInt(v.get(j).toString()) > i) {
							...
							Line2D line1 = new Line2D.Double((primero.getCordY() * sep) + gapX + sep / 2,
									(primero.getCordX() * sep) + gapY + sep / 2,
									(siguiente.getCordY() * sep) + gapX + sep / 2,
									(siguiente.getCordX() * sep) + gapY + sep / 2);

							if (i < numc && sig < numc)
								g2.setColor(new Color(200, 200, 255 - (i * sig) % 120));// vcolors[i%vcolors.length]);
							if (i < numc && sig >= numc)
								g2.setColor(new Color(0, 255 - (i * sig) % 120, 0));// vcolors[i%vcolors.length]);
							if (i >= numc && sig >= numc)
								g2.setColor(new Color(0, 0, 255 - (i * sig) % 120));// vcolors[i%vcolors.length]);
				...
				if (max > 30)
					...
					g2.setStroke(new BasicStroke(4));
				if (i < 10)
					g2.drawString((i + ""), ((float) ((vNodos[i].getCordY() * sep) + gapY + sep / 2)),
							((float) ((vNodos[i].getCordX() * sep) + gapX + sep / 2)));
				else
					g2.drawString((i + ""), ((float) ((vNodos[i].getCordY() * sep) + gapY + sep / 4)),
							((float) ((vNodos[i].getCordX() * sep) + gapX + sep / 2)));
