...
public class ElectricaPanel extends javax.swing.JPanel {
	...
	public void paintComponent(Graphics g) {
				...
				for (int j = 0; j < np; j++)
					if (plano[i][j] != -1) {
						...
						if (i == 0 && j > 0 && plano[i][j] != plano[i][j - 1]) {
							...
							Line2D line2 = new Line2D.Double((i * sepx) + gapX, (j * sepy) + gapY,
									(i * sepx) + sepx + gapX, (j * sepy) + gapY);
							g2.draw(line2);
		...
		// canvi rep np+2
		g2.setPaint(Color.black);
		Line2D line2 = new Line2D.Double((0 * sepx) + gapX, (0 * sepy) + gapY, (0 * sepx) + sepx + gapX,
				(0 * sepy) + gapY);
		g2.draw(line2);
		g2.setPaint(Color.black);
		line2 = new Line2D.Double((0 * sepx) + gapX, ((np) * sepy) + gapY, (0 * sepx) + sepx + gapX,
				((np) * sepy) + gapY);
		g2.draw(line2);
