		...
		for (int i = 0; i < nc; i++) {
			...
			if (newboard.obteUnaDemanda(i).consultaNumPaquet() == -1) {
				...
				while (j < np && !newboard.restriccions(j, i)) {
					// System.out.println("No es pot assignar p"+j+" Q=" +
					// (newboard.obteUnPaquet(j).consultaQini()-newboard.obteUnPaquet(j).consultaQgastada())+"
					// F=" + newboard.obteUnPaquet(j).consultaF() + " P=" +
					// newboard.obteUnPaquet(j).consultaP() + "al c" + i + ": q=" +
					// newboard.obteUnaDemanda(i).consultaQ() + " f=" +
					// newboard.obteUnaDemanda(i).consultaF()+ " p=" +
					// newboard.obteUnaDemanda(i).consultaP());
					j++;
				}
