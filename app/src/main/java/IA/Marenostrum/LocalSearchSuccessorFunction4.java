...
public class LocalSearchSuccessorFunction4 implements SuccessorFunction {
	...
	public List getSuccessors(Object aState) {
		...
		for (int i = 0; i < nProcesos; i++) {
			...
			for (int j = horaMin; j <= horaMax; j++) {
				...
				if (LSBoard.comprobarAsignacion(j, TE)) {
					LocalSearchBoard newLSBoard = new LocalSearchBoard(LSBoard.getHoras(),
							LSBoard.getProcesosSiAsignados(), LSBoard.getProcesosNoAsignados(),
							LSBoard.getHorasInicio(), LSBoard.getHorasLibres(), LSBoard.getProcs());
					newLSBoard.asignarProceso(idProc, j);
					h = LSHeuristicFunction.getHeuristicValue(newLSBoard);

					String S = new String("Asignar " + idProc + "   TE: " + TE + "   h = " + h + "   p = "
							+ newLSBoard.getProcesosSiAsignadosCount());
		...
		for (int i = 0; i < LSBoard.getProcesosSiAsignadosCount(); i++) {
			...
			for (int j = LSBoard.getHoraMinProceso(idProc); j <= LSBoard.getHoraMaxProceso(idProc); j++) {
				...
				if (LSBoard.getHoraInicioProceso(idProc) != j) {
					if (LSBoard.comprobarMovimiento(idProc, j, TE)) {
						...
						String S = new String("Mover " + idProc + "   TE: " + TE + "   h = " + h + "   p = "
								+ newLSBoard.getProcesosSiAsignadosCount());
						listaSucesores.add(new Successor(S, newLSBoard));
					}
				}
			...
			/* Quitamos proceso */
			LocalSearchBoard newLSBoard = new LocalSearchBoard(LSBoard.getHoras(), LSBoard.getProcesosSiAsignados(),
					LSBoard.getProcesosNoAsignados(), LSBoard.getHorasInicio(), LSBoard.getHorasLibres(),
					LSBoard.getProcs());
			newLSBoard.quitarProceso(idProc);
			h = LSHeuristicFunction.getHeuristicValue(newLSBoard);

			String S = new String("Quitar " + idProc + "   TE: " + TE + "   h = " + h + "   p = "
					+ newLSBoard.getProcesosSiAsignadosCount());
			listaSucesores.add(new Successor(S, newLSBoard));
		}
