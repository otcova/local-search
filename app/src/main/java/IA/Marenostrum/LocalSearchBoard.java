...
public class LocalSearchBoard {
	...
	public LocalSearchBoard(int[] pHoras, Vector pSiAsignados, Vector pNoAsignados, int[] pHoraInicio, int pHorasLibres,
			int[] pProcs) {
		horas = new int[HORAS];
		...
		for (int i = 0; i < procesos.size(); i++)
			procesosHoraInicio[i] = pHoraInicio[i];
		for (int i = 0; i < 5; i++)
			procs[i] = pProcs[i];
	...
	public int getRangoCount() {
		int rango = 0;
		for (int i = 0; i < procesos.size(); i++)
			rango = rango + ((Proceso) procesos.get(i)).getRango();
	...
	public void generarEIAleatorio() {
		...
		for (int i = 0; i < procesos.size(); i++) {
			...
			if (comprobarAsignacion(horaInicio, TE))
				asignarProceso(i, horaInicio);
		}
	}
	...
	public void generarEIRango() {
		...
		for (i = 0; i < procesos.size(); i++) {
			switch (((Proceso) procesos.get(i)).getRango()) {
				case 0:
					rango0.add(new Integer(i));
					break;
				case 1:
					rango1.add(new Integer(i));
					break;
				case 2:
					rango2.add(new Integer(i));
					break;
				case 3:
					rango3.add(new Integer(i));
					break;
				case 4:
					rango4.add(new Integer(i));
					break;
				case 5:
					rango5.add(new Integer(i));
					break;
				case 6:
					rango6.add(new Integer(i));
					break;
				case 7:
					rango7.add(new Integer(i));
					break;
				case 8:
					rango8.add(new Integer(i));
					break;
				case 9:
					rango9.add(new Integer(i));
					break;
				case 10:
					rango10.add(new Integer(i));
					break;
			}
		}
	...
	public void generarEIDuracion() {
		...
		for (i = 0; i < procesos.size(); i++) {
			switch (((Proceso) procesos.get(i)).getTE()) {
				case 1:
					duracion1.add(new Integer(i));
					break;
				case 2:
					duracion2.add(new Integer(i));
					break;
				case 3:
					duracion3.add(new Integer(i));
					break;
				case 4:
					duracion4.add(new Integer(i));
					break;
				case 5:
					duracion5.add(new Integer(i));
					break;
			}
		}
	...
	public void asignarProceso(int pIdProc, int pHoraInicio) {
		...
		for (int i = pHoraInicio; i < pHoraInicio + TE; i++)
			horas[i] = pIdProc;
	...
	public void quitarProceso(int pIdProc) {
		...
		for (int i = procesosHoraInicio[pIdProc]; i < procesosHoraInicio[pIdProc] + TE; i++)
			horas[i] = -1;
	...
	public void forzarProceso(int pIdProc, int pHoraInicio) {
		...
		for (int i = pHoraInicio; i < horaFinal; i++) {
			if (horas[i] != -1) {
				if (!solapados.contains(new Integer(horas[i])))
					solapados.add(new Integer(horas[i]));
			}
		}
	...
	public boolean apretarProceso(int pIdProc, int pHoraInicio) {
		...
		if (horas[pHoraInicio] == -1)
			return apretarDerecha(pIdProc, pHoraInicio);
		else {
			...
			if (((horaIniAux + horaFinAux) / (float) 2) < (float) pHoraInicio)
				return apretarIzquierda(pIdProc, pHoraInicio);
			else if (((horaIniAux + horaFinAux) / (float) 2) > (float) pHoraInicio)
				return apretarDerecha(pIdProc, pHoraInicio);
			else {
	...
	private boolean apretarIzquierda(int pIdProc, int pHoraInicio) {
		...
		pos2 = pos1;
		while ((pos2 > 0) && (horas[pos2] != -1))
			pos2--;
		if ((pos2 == 0) && (horas[pos2] != -1))
			return false;
		...
		while (horas[pHoraInicio] != -1) {
			...
			for (int i = pos2 + 1; i <= pos1; i++) {
				...
				if (horaIniAux - 1 < ((Proceso) procesos.get(procIdAux)).getHoraMin())
					return false;
		...
		if (!comprobarAsignacion(pHoraInicio, ((Proceso) procesos.get(pIdProc)).getTE()))
			return apretarDerecha(pIdProc, pHoraInicio);
		else
			asignarProceso(pIdProc, pHoraInicio);
	...
	private boolean apretarDerecha(int pIdProc, int pHoraInicio) {
		...
		if (idAux != -1)
			pos1 = procesosHoraInicio[idAux];
		else {
			pos1 = pHoraInicio;
			while ((pos1 < 168) && (horas[pos1] == -1))
				pos1++;
		}
		...
		pos2 = pos1;
		while ((pos2 < 167) && (horas[pos2] != -1))
			pos2++;
		if ((pos2 == 167) && (horas[pos2] != -1))
			return false;
		...
		while (!comprobarAsignacion(pHoraInicio, ((Proceso) procesos.get(pIdProc)).getTE())) {
			...
			for (int i = pos2 - 1; i >= pos1; i--) {
				...
				if (horaIniAux + 1 > ((Proceso) procesos.get(procIdAux)).getHoraMax())
					return false;
				moverProceso(procIdAux, horaIniAux + 1);
				i = horaIniAux;
			}
